...
module.exports = {
  ...
  async execute(sock, m) {
    ...
    const senderNum = getNumber(senderJid); // الرقم فقط
    const args = m.args || [];
    ...
    await sock.sendMessage(chatId, {
      ...
      caption: `👀 *خمن الشخصية من العين!*\n⏳ الوقت: ${timeMap[inputLevel] / 1000} ثواني فقط!\n🙋‍♂️ اللاعب: @${senderNum}`,
      mentions: [senderJid]
    });
    ...
    const handler = async ({ messages }) => {
      ...
      for (const msg of messages) {
        ...
        if (txt.trim().toLowerCase() === correctAnswer.toLowerCase()) {
          ...
          // تحديث النقاط والرتب باستخدام الرقم
