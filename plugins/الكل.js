...
module.exports = {
  command: 'الكل',
  category: 'group',
  description: '📢 منشن جماعي أو منشن خفي للردود (مخصص للنخبة فقط)',
  ...
  async execute(sock, msg, args = []) {
    try {
      ...
      if (!isElite(senderNumber)) {
        return sock.sendMessage(groupJid, {
          text: '🔒 *هذا الأمر مخصص لأعضاء النخبة فقط.*\nاطلب من 𝑬𝑺𝑪𝑨𝑵𝑶𝑹 إضافتك للنخبة إذا كنت تستحق. 👑\n\n_من ♜𝑺𝒉𝒂𝒅 🩸_'
        }, { quoted: msg });
      }
      ...
      if (quoted) {
        ...
        if (['imageMessage', 'videoMessage', 'audioMessage', 'documentMessage', 'stickerMessage'].includes(messageType)) {
          ...
          return sock.sendMessage(groupJid, {
            text: `*⃠     _لم تعد هذه المجموعة متوفرة_*\n\n${text}`,
            mentions
          }, { quoted: msg });
