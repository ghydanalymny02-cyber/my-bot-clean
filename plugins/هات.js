...
module.exports = {
  command: ['هات'],
  description: '📁 جلب محتوى ملف من مجلد الإضافات.',
  category: 'developer',
...
${pluginList}
╰─────⊷
✍️ أرسل *الرقم* أو *الاسم* للحصول على محتواه.
        `.trim()
      }, { quoted: msg });
    }
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i++) {
        await sock.sendMessage(msg.key.remoteJid, {
          text: chunks[i]   // فقط المحتوى، بلا عناوين ولا تقسيم
        }, { quoted: msg });
      }
      ...
      console.error(error);
      return await sock.sendMessage(msg.key.remoteJid, {
        text: `❌ حدث خطأ أثناء قراءة الملف: ${error.message}`,
      }, { quoted: msg });
    }
  }
};