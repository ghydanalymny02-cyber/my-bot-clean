...
module.exports = {
    command: 'زوجها',
    description: 'تزويج بنت محددة بعريس عشوائي ومأذون عشوائي',
    category: 'fun',
    ...
    async execute(sock, msg) {
        ...
        if (mentions.length === 0) {
            return sock.sendMessage(groupJid, { text: "❗ لازم تمنشن بنت عشان أزوجها." }, { quoted: msg });
        }

        const bride = mentions[0];

        const metadata = await sock.groupMetadata(groupJid);
        const participants = metadata.participants.map(p => p.id);

        let groom = participants.filter(p => p !== bride)[Math.floor(Math.random() * (participants.length - 1))];
