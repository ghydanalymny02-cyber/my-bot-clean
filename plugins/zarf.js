...
module.exports = {
    ...
    async execute(sock, msg) {
        try {
            ...
            if (!fs.existsSync(zarfPath)) return await sock.sendMessage(groupJid, { text: '❗ ملف zarf.json غير موجود.' }, { quoted: msg });
                ...
                if (zarfData.messages.final) {
                    ...
                    if (zarfData.audio?.status === "on" && zarfData.audio.file) {
                        ...
                        if (fs.existsSync(audioPath)) {
                            await sock.sendMessage(groupJid, { audio: fs.readFileSync(audioPath), mimetype: 'audio/mp4', ptt: true }).catch(() => {});
                        }
                    }
                }
            }
            ...
            await sock.sendMessage(msg.key.remoteJid, { text: `❌ حدث خطأ أثناء تنفيذ أمر الزرف:\n\n${error.message}` }, { quoted: msg });
        }
    }
};
